...
- Created external CSS file and linked it to HTML
- Applied typography styling with different font sizes and line heights
- Implemented spacing using both margin and padding
- Styled header and footer with background colors
- Created horizontal navigation with hover effects
- Made images responsive with max-width
- Centered and constrained main content width
- Added a non-required additional centered figure class out of personal preference
- I corrected a minor mistake with the implementation of the `aria-labelledby`.

## CSS selectors I used (at least 5)
- **Element selector**: `body`, `h1`, `h2`, `p`, `img`
- **Group selector**: `header, footer`
- **Descendant selector**: `nav a`, `nav ul`, `nav li`
- **Class selector**: `.card`, `.centered-figure`
- **ID selector**: `#works`
- **Pseudo-class**: `a:hover`, `a:visited`

## One thing I struggled with
- I mildly struggled with centering the figures. Fixed it per my personal preference.

## One improvement I want to do next
- I would really like to add more color schemes and improve visual hierarchy.